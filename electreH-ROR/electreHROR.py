        ...
        for crit in sort:
            if crit.level < max :
                ...
                for alt1 in self.alternativesIDs:
                    row = ""
                    for alt2 in self.alternativesIDs:
                        cmpr = self.compareTwoAlternativesROR(alt1, alt2, crit, criteria, cutLevel)
                        matrixAlt1.setdefault(alt1, {}).update({alt2:cmpr['S']})
                    
                values[crit] = matrixAlt1
        self.outrankingMatrix = values
        
    def compareTwoAlternativesROR(self, alt1, alt2, atCriterion, criteria, cutLevel):
        if alt1==alt2:
            return {'S':'?'}
...
#def parse_xmcda_files(in_dir):
    ...
    try:
        ...
        criteria = getHierarchyArray(xml_hierarchy)
